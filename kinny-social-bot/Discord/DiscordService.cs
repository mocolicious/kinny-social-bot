...
namespace kinny_social_bot.Discord
{
    internal class DiscordService : SocialService<SocketCommandContext, DiscordCredentials>
    {
        ...
        {
            if (!(sm is SocketUserMessage sum))
            {
                return;
            }

            SocketCommandContext context = new SocketCommandContext(_client, sum);
        ...
        {
            bool containsSlashCommand = context.Message.Content.ToLower().Contains($"/{_client.CurrentUser.Username} ");
            bool isBotMentioned = context.Message.MentionedUsers.Any(u => u.Username == _client.CurrentUser.Username);
